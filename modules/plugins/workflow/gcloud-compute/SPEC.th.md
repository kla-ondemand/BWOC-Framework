...
> [!abstract] ปลั๊กอินอ้างอิงสาย `workflow` ตัวแรกที่ **เขียนได้** ของตระกูล GCP (`BWOC-EPIC-9`) รับผิดชอบ **วงจรชีวิตของ Compute Engine instance** — แสดงรายการ instance, สั่ง start, สั่ง stop คำสั่ง: `list` (อ่าน), `start` / `stop` (**เขียน — มี gate ยืนยันโดยผู้ดูแลที่ระดับ CLI** และยังถูก guard ด้วย confirmation marker `confirmed: true` ใน request ที่ CLI ตั้งไว้) `delete` ตั้งใจ **ไม่** ส่งมา (เลื่อนออกไป — กู้คืนไม่ได้) source ฟังก์ชันช่วยด้านข้อมูลรับรองจากปลั๊กอินพี่น้อง [[../gcloud-auth/SPEC.th|`gcloud-auth`]] framing ฉบับเต็ม: [[../../../notes/2026-05-28_gcloud-compute-write-verbs|บันทึกออกแบบ BWOC-66]]
...
มัน **มี** guard เชิงป้องกันเชิงลึก (บันทึก §Decision 3): verb เขียนปฏิเสธที่จะทำงานเว้นแต่ confirmation marker `confirmed: true` ที่ CLI ตั้งไว้ปรากฏใน request (CLI `bwoc gcloud compute` ใส่ให้เฉพาะหลัง gate ยืนยันโดยผู้ดูแลผ่านแล้ว — BWOC-68 `compute_write_request`) ดังนั้นการเรียกปลั๊กอินโดยตรง — ข้าม gate ของ CLI — จะถูกปฏิเสธด้วย envelope "no change" ที่มีโครงสร้าง (`ok: false`, `changed: false`, `reason: "unconfirmed-write"`) แทนที่จะเขียนเงียบ ๆ หรือ fail แบบเปล่า ๆ (ธัมมานุปัสสนา) marker คือการ coupling เพียงอย่างเดียวระหว่าง gate กับปลั๊กอิน ส่วนตาราง `[[verb]]` ใน manifest ประกาศว่า verb ใดเป็นการเขียนเพื่อให้ทั้ง `bwoc check` (BWOC-70) และ gate ของ CLI มองเห็น classification
...
| stdin | คำขอ JSON บรรทัดเดียว เช่น `{"operation":"list"}`, `{"operation":"start","instance":"vm-1","zone":"us-central1-a","confirmed":true}`, `{"operation":"stop","instance":"vm-1","zone":"us-central1-a","project":"my-proj","confirmed":true}` |
| `.confirmed` (ใน JSON บน stdin) | confirmation marker — ตั้งเป็น `true` โดย CLI **หลังจาก** ผู้ดูแลยืนยันการเขียน verb อ่านไม่สนใจมัน; `start` / `stop` ปฏิเสธถ้าไม่มี |
...
ปล่อยออกมา (ออกด้วยรหัส `5`) เมื่อ verb เขียนถูกเรียกโดยไม่มี `confirmed: true` ใน request ในเส้นทาง CLI ปกติ marker จะถูกตั้งเสมอหลังผู้ดูแลยืนยัน ดังนั้นกรณีนี้จะเกิดเฉพาะตอน direct-invoke ที่พยายาม bypass
...
| `5` | unconfirmed-write | verb เขียน (`start` / `stop`) ถูกเรียกโดยไม่มี marker `confirmed: true` — ปฏิเสธ ไม่มีการเปลี่ยนแปลง |
