...
_A real OS-level sandbox (landlock / `sandbox-exec`, replacing the v1 stub) and `bwoc-harness` Windows support continue here. Trust v2 **warn-mode** shipped (below); cryptographic **signed envelopes** are deferred — `trust.md` defines v2 as warn-mode, so signing is not pursued until a concrete cross-workspace need (give-feedback / network transport) drives it. See [`docs/en/ROADMAP.en.md`](docs/en/ROADMAP.en.md) §Phase 3._
...
- **`bwoc peer` — read-only cross-workspace view (issue #20)** — `bwoc peer list` shows peers declared in `.bwoc/interconnect/routes.toml`; `bwoc peer status <key>` resolves a peer via `Routes` and reads (read-only, local FS) its agents (`AgentsRegistry`) and Saṅgha open tasks. Reuses the existing loaders pointed at the peer root — no new parsing, no new deps. The epic's *learn* (allowlist-gated ingestion) and *give-feedback* (write, needs cross-workspace identity) paths are deferred. Realises Oracle's cross-mesh state-sensing — **Kalyāṇamitta / Samānattatā / Anattā** (no central broker).
