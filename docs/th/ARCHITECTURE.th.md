...
สำหรับ stack **เชิงแนวคิด** (กรอบพุทธ ๒๒ ประการ) ดูที่ [`PHILOSOPHY.th.md`](../../modules/agent-template/docs/th/PHILOSOPHY.th.md) เอกสารนี้พูดถึง **implementation**
...
### ๑. Framework Repository

repo นี้ บรรจุ —

- สเปก Markdown — `AGENTS.md`, การ map กรอบ ๒๒ ประการใน `PHILOSOPHY.th.md`, `PRD`, `SRS`, `THREAT-MODEL` ฯลฯ
...
### ๒. Agent Template — `modules/agent-template/`

พิมพ์เขียวที่ถูก copy ไปยังทุก agent ใหม่ บรรจุ —

- **`AGENTS.md`** — แหล่งความจริงเดียวที่เป็นกลางต่อ backend
- **Backend symlinks** — `CLAUDE.md`, `GEMINI.md`, `CODEX.md`, `KIMI.md` ล้วนชี้ไปที่ `AGENTS.md`
...
### ๕. การ Execute Backend

โมเดล Phase 1: **`bwoc spawn` ทำการ exec backend CLI ที่กำหนด** (Claude Code, Gemini CLI, Codex CLI, Kimi CLI) ภายใน directory ของ agent backend อ่าน `AGENTS.md` ผ่าน backend file ของมัน (ซึ่ง symlink ไป) แล้วทำงานตามสเปก
...
```
๑. User                  bwoc spawn agent-foo
๒. CLI                   resolve `agent-foo` เป็น directory บนดิสก์
                         (search path: cwd → ~/bwoc-agents/ → $BWOC_PATH)
๓. CLI                   อ่าน agent-foo/config.manifest.json
                         resolve {{primaryBackend}}, {{primaryModel}}
๔. CLI                   `cd agent-foo && exec <backend-cli>`
                         (เช่น `exec claude code`)
๕. Backend CLI           อ่าน AGENTS.md (ผ่าน entry file ที่ symlink)
                         ใช้ persona, manifest, capabilities
๖. Agent                 ทำงานตามวงจรอริยสัจ ๔
                         เพิ่มรายการลงใน task-log.jsonl
๗. Agent                 เมื่อจบ exit; หรือ
...
| CLI → Backend subprocess | Untrusted args ส่งต่อไป LLM context | THREAT-MODEL §๑ |
| Backend → `AGENTS.md` | Direct prompt injection | THREAT-MODEL §๑.๑ |
| Backend → อ่านเนื้อหาไฟล์ | Indirect prompt injection | THREAT-MODEL §๑.๒ |
| Agent → memory files | Social engineering ผ่าน memory ที่ปลูกไว้ | THREAT-MODEL §๑.๓ |
| Agent ↔ Agent (Phase 3) | Capability spoofing | THREAT-MODEL §๑.๔ |

การกระทำที่ห้ามพื้นฐาน (ศีล ๕) และประเภทภัยตามตัณหา (ตัณหา ๓) เป็นฐานทางหลักการ ดู `SECURITY.md` สำหรับขั้นตอนรายงาน

---

## ดูเพิ่ม

- [`PHILOSOPHY.th.md`](../../modules/agent-template/docs/th/PHILOSOPHY.th.md) — stack เชิงแนวคิดและการ map กรอบ ๒๒ ประการ
- [`PHILOSOPHY.th.md §๐.๑`](../../modules/agent-template/docs/th/PHILOSOPHY.th.md#๐๑-วงรอบ--อุปฺปาท--ฐิติ--วยะ) — วงรอบ (อุปฺปาท · ฐิติ · วยะ) ที่ใช้จัด CLI commands
