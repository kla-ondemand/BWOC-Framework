...
|---|---|---|---|---|
| `query` | อ่าน | ต้องมี | `GET /rest/api/3/search/jql` (JQL จำกัด project, แบ่งหน้าด้วย token) | ไม่มี — การอ่านไม่มีต้นทุน |
...
- **ชุดผลลัพธ์มีขอบเขต** `maxResults` ถูกจำกัดที่ ≤ 100 และดึงทีละหน้าเดียว (`/search/jql` แบ่งหน้าด้วย token ผ่าน `nextPageToken`/`isLast` ซึ่งส่งกลับมาใน response เผื่อทำ paging loop ในอนาคต) — อะแดปเตอร์ไม่เคยดึงข้อมูลแบบไม่มีขอบเขต นี่เป็นแนวป้องกัน rate limit ด่านแรก
